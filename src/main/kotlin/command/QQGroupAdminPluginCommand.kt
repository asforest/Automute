package org.example.qqgroupadmin.command
    ...
    secondaryNames = arrayOf("adm", "ga"))
{
    ...
    private suspend fun CommandSender.runWithPermission(block: suspend CommandSender.() -> Unit)
    {
        if(hasPermission(QQGroupAdminPluginMain.permssion_mainCommand))
            block()
//        if(user == null || user!!.id in PluginConfig.admins)
//            block()
    }
